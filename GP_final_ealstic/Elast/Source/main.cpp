...
{	
	...
	Vector<int> bc_lo_dis(2,0);
	Vector<int> bc_lo_str(2,0);
	...
	Vector<int> bc_hi_dis(AMREX_SPACEDIM,0);
	...
	bc_hi_dis[X] = stod(bound[0][1]);
	bc_hi_dis[Y] = stod(bound[0][3]);
	bc_lo_dis[X] = stod(bound[0][2]);
	bc_lo_dis[Y] = stod(bound[0][4]);
	#if(AMREX_SPACEDIM>2)
	bc_hi_dis[Z] = stod(bound[0][5]);
	bc_lo_dis[Z] = stod(bound[0][6]);
	#endif

	bc_hi_str[X] = stod(bound[0][1]);
	bc_hi_str[Y] = stod(bound[0][3]);
	bc_lo_str[X] = stod(bound[0][2]);
	bc_lo_str[Y] = stod(bound[0][4]);
	#if(AMREX_SPACEDIM>2)
	bc_hi_str[Z] = stod(bound[0][5]);
	bc_lo_str[Z] = stod(bound[0][6]);
	#endif
	...
	Vector<BCRec> bc_dis(disp_X.nComp());
	...
	bound_cond(disp_X, bc_dis,bc_hi_dis,bc_lo_dis);
	bound_cond(disp_Y, bc_dis,bc_hi_dis,bc_lo_dis);
	...
	//Init_comp(phi_new,comp_new);
	Calculate_Tau(phi_new);
	...
	//phase.push_back("mu_Al");
	//Print()<<"3done\n";
	//phase.push_back("Composition");
	...
	for(n=stepnum+1; n<=nsteps; ++n)
	 {  
		...
		//MultiFab::Copy(comp_old, comp_new, 0,0,1,0);
		//Print()<<"in for\n";
		...
		FillDomainBoundary(disp_X, geom, bc_dis);
		FillDomainBoundary(disp_Y, geom, bc_dis);
		...
		Iterative_stress_solver(phi_old,disp_X,disp_Y,strain_X, strain_Y);
        ...
        //Advance the solution to calculate field variables for next time step----------------------------------------------------------------------------------
	 	advance(phi_old, phi_new, mu_old, mu_new, comp_old, comp_new, term1, term2, term3, term4, psi, disp_X, disp_Y, lambad,geom);
