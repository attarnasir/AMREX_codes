...
void update_phi(MultiFab& phi_new, MultiFab& phi_old, MultiFab& term1, MultiFab& term2, MultiFab& term3, MultiFab& term4, MultiFab& lambad, Geometry const& geom)
{	
	...
	for (MFIter mfi(phi_old); mfi.isValid(); ++mfi)
	{
		...
		{	
			...
			for(int a=0; a<numphase; a++){
				...
				if(fabs(div(a))>0.0){
					deltaphi(a) = (time_step/(epsilon*FunctionTau(i,j,k,phiOld)))*(lamb(i,j,k,a)-sum_lambda);
				}
				else{
					deltaphi(a) = 0.0;
				}
