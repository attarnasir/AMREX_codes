...
{       
    Print()<<"In readc\n";
        ...
        for(int a=0; a<nump-1;a++){
            ...
            //fout.open("constant/Composition_NiAlMo_"+ phasemap[a] +".csv");
...
void getc(){
    ...
    for(int a=0; a<nump; a++){
        for(int i=0; i<numcom-1; i++){
            ...
            Print()<<"For norm: i: "<<i<<" , a: "<<a<<" : "<<findC(a,T,i)<<"\n";
            Print()<<"For eq: i: "<<i<<" , a: "<<a<<" : "<<findC(a,Teq,i)<<"\n";
...
{       
        ...
        {   
            ...
            //fout.open("constant/HSN_NiAlMo_"+ phasemap[a] +".csv");
...
void function_F_04_c_mu(int i, int j, int k, amrex::Array4<Real const> const& mu, Array2D<Real,0,phasecount-1,0,compcount-2, Order::C> &c, Array2D<Real,0,phasecount-1,0,compcount-2, Order::C> BB, Array3D<Real,0,phasecount-1,0,compcount-2,0,compcount-2, Order::C> AA, int numcomp, int a){


            // for(int p=0; p<numcomp-1;p++){
            //     for(int h=0; h<numcomp-1; h++){
            //         Print()<<"A["<<a<<","<<p<<","<<h<<"]:"<<AA(a,p,h)<<"\n"; 
            //     }
            // }
            ...
            Array2D <Real,0,compcount-2,0,compcount-2> cmu{};
                ...
                // for(int p=0; p<numcomp-1;p++){
                //     for(int h=0; h<numcomp-1; h++){
                //         Print()<<"muc["<<p<<","<<h<<"]:"<<muc(p,h)<<"\n"; 
                //     }
                // }

                //mat_inv(muc,cmu,numcomp-1,a);
                if(numcomp==3){
                ...
                // mat_inv(muc_eq,cmu_eq,numcomp-1,a);

                // for(int p=0; p<numcomp-1;p++){
                //     for(int h=0; h<numcomp-1; h++){
                //         Print()<<"cmu["<<p<<","<<h<<"]:"<<cmu(p,h)<<"\n"; 
                //     }
                // }

                //Print()<<"mu["<<i<<","<<j<<","<<k<<",m]:"<<mu(i,j,k,0)<<"\n";
                ...
                for(int l=0; l < numcomp-1; l++) {
                    ...
                    for (int m=0; m < numcomp-1; m++) {
                        sum += cmu(l,m)*(mu(i,j,k,m)-BB(a,m)); ////check this mu has spatial components as well
                    }
                    c(a,l) = sum;
                    sum=0.0;
                }
    ...
    dcdmu_eq = Vector<Vector<Vector<Real>>>(nump,Vector<Vector<Real>>(numcom-1,Vector<Real>(numcom-1,0.0)));
     ...
     for(int a=0; a<nump; a++){
        ...
        Array2D <double,0,compcount-2,0,compcount-2> muc_eq{};
                ...
                for(int l=0; l<numcom-1;l++){
                    for(int m=0; m<numcom-1; m++){
                        Print()<<"dcdmu["<<a<<"]["<<l<<"]["<<m<<"] : "<<dcdmu[a][l][m]<<"\n";
                        Print()<<"dcdmueq["<<a<<"]["<<l<<"]["<<m<<"] : "<<dcdmu_eq[a][l][m]<<"\n";
                    }
                }
     }
...
AMREX_GPU_DEVICE AMREX_FORCE_INLINE
void function_F_04_free_energy(int i, int j, int k, Array3D<Real,0,phasecount-1,0,compcount-2,0,compcount-2, Order::C> AA, Array2D<Real,0,phasecount-1,0,compcount-2, Order::C> BB, Array1D <Real,0,phasecount-1> CC, Array1D<Real,0,phasecount-1> &fe ,Array2D<Real,0,phasecount-1,0,compcount-2, Order::C> &c, int numcomp, int a){
