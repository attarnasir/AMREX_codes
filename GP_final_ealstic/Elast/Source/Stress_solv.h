#ifndef STRESS_SOLV_H
#define STRESS_SOLV_H
...
void calculate_stiffness(int i, int j, int k, int numphase,Array2D <Real, 0, phasecount-1,0,3> stiff_phase, Array2D<Real,0,AMREX_SPACEDIM-1,0,AMREX_SPACEDIM> &stiffness_c, amrex::Array4<Real const> const& phi){
    for(int a=0; a<numphase; a++){
        stiffness_c(X,0) += stiff_phase(a,1)*phi(i,j,k,a);
        stiffness_c(X,1) += stiff_phase(a,2)*phi(i,j,k,a);
        stiffness_c(X,2) += stiff_phase(a,3)*phi(i,j,k,a);

        stiffness_c(Y,0) += stiff_phase(a,1)*phi(i,j,k,a);
        stiffness_c(Y,1) += stiff_phase(a,2)*phi(i,j,k,a);
        stiffness_c(Y,2) += stiff_phase(a,3)*phi(i,j,k,a); 
    }
}


void Iterative_stress_solver(MultiFab& phi_old, MultiFab& disp_X, MultiFab& disp_Y, MultiFab& strain_X, MultiFab& strain_Y){
    ...
    strain_X.FillBoundary();
	strain_Y.FillBoundary();
    ...
    for ( MFIter mfi(phi_old); mfi.isValid(); ++mfi )
    {
		...
		{   
            Array2D<Real,0,AMREX_SPACEDIM-1,0,AMREX_SPACEDIM> eg_st{};
            Array2D<Real,0,AMREX_SPACEDIM-1,0,AMREX_SPACEDIM> strain{};         //strain[X][xx] = 0,0 and so on
            ...
            lambda_ipo = stiffness_c_ipo(X,1);             //stiffness(,0)=c11, stiffness(,1)=c12, stiffness(,2)=c44
            lambda_imo = stiffness_c_imo(X,1);
            lambda_jpo = stiffness_c_jpo(Y,1);
            lambda_jmo = stiffness_c_jmo(Y,1);

            mu1_ipo = stiffness_c_ipo(X,2);
            mu1_imo = stiffness_c_imo(X,2);
            mu1_jpo = stiffness_c_jpo(Y,2);
            mu1_jmo = stiffness_c_jmo(Y,2);

            mu1_prime_ipo = stiffness_c_ipo(X,0)-stiffness_c_ipo(X,1)-2.0*stiffness_c_ipo(X,2);
            mu1_prime_imo = stiffness_c_imo(X,0)-stiffness_c_imo(X,1)-2.0*stiffness_c_imo(X,2);
            mu1_prime_jpo = stiffness_c_jpo(Y,0)-stiffness_c_jpo(Y,1)-2.0*stiffness_c_jpo(Y,2);
            mu1_prime_jmo = stiffness_c_jmo(Y,0)-stiffness_c_jmo(Y,1)-2.0*stiffness_c_jmo(Y,2);
