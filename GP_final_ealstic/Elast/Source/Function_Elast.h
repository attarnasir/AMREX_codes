#ifndef FUNCTION_ELAST_H
#define FUNCTION_ELAST_H
...
BL_PROFILE("computeterm2()");
	...
	for ( MFIter mfi(phi_old); mfi.isValid(); ++mfi )
    {
        ...
        {	
            ...
            sigma(xx) = stiff(0)*strain(xx) + stiff(1)*strain(yy);
            sigma(yy) = stiff(1)*strain(xx) + stiff(0)*strain(yy);
            sigma(xy) = 2.0*stiff(2)*strain(xy);
